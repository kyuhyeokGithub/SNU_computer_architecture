...
class EX(Pipe):
    ...
    def log(self):
        ...
        ALU_OPS = {
            ...
            # Express these are snu-vector as < _ >
            # Express add, sub, mul as +,-,*
            # Express using imm as .
            # Express these are unpack, pack, brdcst using ::, :, ; after sv and elementary operation "~"
            ALU_SVUNPACK   : f'# {self.alu_out:#010x} <- {self.op1_data:#010x} <~::> {self.alu2_data:#010x}',
            ALU_SVPACK     : f'# {self.alu_out:#010x} <- {self.op1_data:#010x} <~:> {self.alu2_data:#010x}',
            ALU_SVBRDCST   : f'# {self.alu_out:#010x} <- {self.op1_data:#010x} <~;> {self.alu2_data:#010x}',
            ALU_SVADDI     : f'# {self.alu_out:#010x} <- {self.op1_data:#010x} <+.> {self.alu2_data:#010x}',
            ALU_SVADD      : f'# {self.alu_out:#010x} <- {self.op1_data:#010x} <+> {self.alu2_data:#010x}',
            ALU_SVSUB      : f'# {self.alu_out:#010x} <- {self.op1_data:#010x} <-> {self.alu2_data:#010x}',
            ALU_SVMUL      : f'# {self.alu_out:#010x} <- {self.op1_data:#010x} <*> {self.alu2_data:#010x}',
        }
